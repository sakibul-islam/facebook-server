...
const friendSuggestions = Object.keys(profiles).filter((key, i) => (i % 2)).map(key => profiles[key])

const profileArr = Object.keys(profiles).map(key => profiles[key])

const friendRequests = Object.keys(profiles).filter((key, i) => !(i % 2)).map(key => profiles[key])
